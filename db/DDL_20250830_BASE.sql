...
/* ============================================================
   RORO_MAP_FAVORITE 作成（1215対策つき：親テーブルを整えてから作成）
   環境: MySQL/MariaDB (phpMyAdmin でそのまま実行可)
   前提: RORO_CUSTOMER が存在していること
============================================================ */

-- 親を InnoDB に（既に InnoDB でも安全に実行可）
ALTER TABLE RORO_CUSTOMER ENGINE=InnoDB;

-- 親の型を INT（符号付き）に明示（既に INT でも安全）
ALTER TABLE RORO_CUSTOMER
  MODIFY customer_id INT NOT NULL;

-- 親に PRIMARY KEY が無ければ付与（あれば何もしない）
SET @has_pk := (
  SELECT COUNT(*)
...
SET @sql := IF(@has_pk = 0,
  'ALTER TABLE RORO_CUSTOMER ADD PRIMARY KEY (customer_id);',
  'DO 0'
);
PREPARE s FROM @sql; EXECUTE s; DEALLOCATE PREPARE s;

-- 念のため外部キー検査をオフ（作成時の相互依存回避用）
SET @fk_old := @@FOREIGN_KEY_CHECKS;
SET FOREIGN_KEY_CHECKS = 0;

-- 既存があれば削除
DROP TABLE IF EXISTS RORO_MAP_FAVORITE;

-- 子テーブル（親の INT に合わせる）
CREATE TABLE RORO_MAP_FAVORITE (
  favorite_id  BIGINT       NOT NULL AUTO_INCREMENT,
  customer_id  INT          NOT NULL,  -- 親と完全一致（INT/符号付き）
...
/* === トラブル時の確認用（必要なら別途実行）
SHOW CREATE TABLE RORO_CUSTOMER\G
